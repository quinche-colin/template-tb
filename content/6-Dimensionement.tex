...
Afin d'analyser cette saccade, il est nécessaire de connaitre la différence de hauteur de tige des différents mouvements, car il s'agit de la seule dimensions pouvant varier d'un mouvement à l'autre avec cette solution de fixation. 
...
Une fois le modèle géométrique défini, il est possible de représenté l'évolution de l'angle $\beta$ en fonction de $\alpha$ le tout pour différentes valeurs de désalignement. 
\begin{figure}[H]
  \centering
...
De plus, il est également possible de représenté la différence entre ces deux angles.
...
(\autoref{fig:profil_remontage}, \autoref{fig:profil_vieillissement}). Se saut est absorbé mécaniquement par le jeu angulaire de
l'accouplement.
...
correspondant à la réserve de marche totale $RM$. Les deux valeurs étant connu, il est possible de calculé le nombre d'heure corespondant à un tour de
