...
Couple net additionnel (au-delà de $C_{\text{détente}}$) que le moteur doit transmettre à la tige pour entraîner le mouvement, c'est-à-dire vaincre le couple du barillet ramené sur la tige. La valeur est calculée pour chaque mouvement :
\begin{equation}
    C_{\text{requis},i} = \frac{C_{\text{barillet},i}}{r_i}
...
\begin{table}[H]
  ...
  \begin{tabular}{|c|c|c|c|}
    \hline
    \textbf{Mouvement} & $\boldsymbol{C_{\text{barillet}}}$ \textbf{[mN·m]} & $\boldsymbol{r}$ \textbf{(tige/bar.)} & $\boldsymbol{C_{\text{requis}}}$ \textbf{[mN·m]} \\
    \hline
    Mvt19    & 6,7  & 3,33 & 2,01 \\
    \textbf{Mvt01}    & \textbf{10,8} & \textbf{3,33} & \textbf{3,24} \\
...
Le cas critique est le \textbf{Mvt01} ($C_{\text{barillet}} = 10{,}8$ mN·m, $r = 3{,}33$), qui donne le $C_{\text{requis}}$ le plus élevé :
\begin{equation}
    C_{\text{requis}} = \frac{C_{\text{barillet,max}}}{r}
    = \frac{10{,}8}{3{,}33}
...
\begin{itemize}
    \item \textbf{Régime accouplement} ($C_{\text{détente}} < C < C_{\text{détente}} + C_{\text{requis}}$)~: le couple moteur dépasse le seuil de détente, l'arbre tourne, mais le couple net transmis à la tige reste inférieur à $C_{\text{requis}}$. Le moteur s'accouple mécaniquement sans entraîner le mouvement.
    \item \textbf{Régime remontage / dévidage} ($C_{\text{détente}} + C_{\text{requis}} < C < C_{\text{max,tige}}$)~: le couple net transmis dépasse $C_{\text{requis}}$ et le moteur entraîne activement la tige, permettant le remontage ou le dévidage contrôlé du barillet.
\end{itemize}
...
\begin{tikzpicture}[font=\small]
  % Positions (échelle non proportionnelle, cas critique Mvt01) :
  % 0→0,  5→2.5,  8,24→5.0,  15→8.5,  55→13
  ...
  % Graduations
  \foreach \xpos/\lab in {0/0, 2.5/5, 8.5/15, 13.0/55}{
    \draw (\xpos, 0.15) -- (\xpos, -0.15);
    \node[below] at (\xpos, -0.2) {\lab};
  }
  \draw (5.0, 0.15) -- (5.0, -0.15);
  \node[below] at (5.0, -0.2) {8{,}24};
