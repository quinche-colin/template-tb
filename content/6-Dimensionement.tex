...
Le cas critique est le Mvt01 ($C_{\text{barillet}} = 10{,}8$ mN·m, $r = 3{,}33$), qui donne le $C_{\text{requis}}$ le plus élevé :
\begin{equation}
...
\begin{itemize}
    \item Régime accouplement ($C_{\text{détente}} < C < C_{\text{détente}} + C_{\text{requis}}$)~: le couple moteur dépasse le seuil de détente, l'arbre tourne, mais le couple net transmis à la tige reste inférieur à $C_{\text{requis}}$. Le moteur s'accouple mécaniquement sans entraîner le mouvement.
    \item Régime remontage / dévidage ($C_{\text{détente}} + C_{\text{requis}} < C < C_{\text{max,tige}}$)~: le couple net transmis dépasse $C_{\text{requis}}$ et le moteur entraîne activement la tige, permettant le remontage ou le dévidage contrôlé du barillet.
\end{itemize}
