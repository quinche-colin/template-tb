...
tour complet, en deux colonnes de quatre cercles (colonne 1~: $\alpha=0°$ à
$135°$~; colonne 2~: $\alpha=180°$ à $315°$), en reprenant la construction à
deux cercles de rayon $R$ (un autour de $O_1$, un autour de $O_2$) et en
faisant apparaître le désalignement $e$ entre les deux centres. Les deux
points où le bras moteur croiserait le cercle moteur sont toujours
représentés (symétrie oblige)~; celui qui est effectivement actif -- entouré
sur la figure -- est en haut pour $\alpha<180°$ et en bas pour
$\alpha\geq180°$. On observe que la seconde colonne ($180°$ à $315°$)
reproduit exactement les mêmes valeurs de $\Delta$ que la première ($0°$ à
$135°$) : c'est la périodicité de 180° de l'\eqref{eq:beta_periodicity}. Le
système étant entièrement décrit par un seul demi-tour, les
\autoref{fig:beta_evolution} et \autoref{fig:delta_evolution} qui suivent se
limitent à $\alpha\in[0°,180°]$.
...
\begin{figure}[H]
  ...
  complet, en deux colonnes de quatre. Les axes $O_1$ (moteur) et $O_2$
  (tige) et leur désalignement $e$ sont indiqués sur le premier panneau ; le
  bras moteur actif est entouré. $R=12{,}5$~mm, $e=\Delta h=3{,}18$~mm.}
...
\begin{figure}[H]
  ...
  \caption{Écart angulaire $\Delta=\beta-\alpha$ sur un demi-tour, pour
...
Plutôt qu'un profil trapézoïdal générique, la \autoref{fig:profil_saccade}
superpose la saccade périodique (\eqref{eq:omega_ratio}, prolongée par
périodicité de 180° comme l'\eqref{eq:beta_periodicity}) aux deux profils de
vitesse réellement utilisés par le logiciel, avec l'accélération maximale de
5000~tr/min\textsuperscript{2} déjà retenue pour le dimensionnement
dynamique de l'accouplement (\eqref{eq:alpha})~:

\begin{itemize}
  \item \textbf{Remontage} (mode réserve de marche, cas du Mvt01)~: 50~tr/min,
  sur $N_{\text{tige}} = 34{,}3$~tr (\eqref{eq:n_tige_mvt01}) -- soit une
  trentaine de tours. Les phases d'accélération et de décélération, très
  courtes ($0{,}25$~tr chacune), laissent place à un long palier à vitesse
  constante où la saccade se répète une soixantaine de fois.
  \item \textbf{Vieillissement accéléré}~: 350~tr/min visés, sur seulement
  2~tr. Cette distance est très inférieure à celle nécessaire pour atteindre
  350~tr/min à cette accélération ($12{,}25$~tr) -- le profil réel est donc
  \emph{triangulaire}, avec un pic à seulement $100$~tr/min.
\end{itemize}

Conformément à l'\eqref{eq:eps_bounds_calc}, la vitesse instantanée croît
jusqu'à $+34\ \%$ au-dessus de la consigne puis chute brusquement à $-20\ \%$
en dessous à chaque changement de bras actif (tous les 180° de rotation
moteur, soit deux fois par tour), quel que soit le profil de consigne.

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{assets/figures/Implementation/profil de vitesse trapezoidale avec saccade.png}
  \caption{Profils de vitesse réels avec saccade périodique due au
  désalignement réel de l'accouplement, $e=\Delta h=3{,}18$~mm~: remontage
  du Mvt01 (gauche) et vieillissement accéléré (droite, profil
  triangulaire). Le motif se répète tous les 180° de rotation moteur.}
