...
$\Delta\theta(\alpha)$ atteint son maximum quand sa dérivée s'annule,
c'est-à-dire quand $d\beta/d\alpha=1$. D'après l'équation~\eqref{eq:omega_ratio},
cela revient à $\cos\alpha=-e/R$. En remplaçant cette valeur dans
l'équation~\eqref{eq:beta_alpha}, le dénominateur de l'atan2 devient nul, ce
qui donne exactement $\beta=\pi/2$~: l'écart maximal se produit donc toujours
au moment où la tige passe par 90°, d'où :

\begin{equation}
  \Delta\theta_{\text{max}} = \frac{\pi}{2} - \arccos\!\left(-\frac{e}{R}\right) = \arcsin\!\left(\frac{e}{R}\right)
  \label{eq:delta_max}
\end{equation}
...
\begin{equation}
  \Delta\theta_{\text{max}} = \arcsin\!\left(\frac{3{,}18}{12{,}5}\right) \approx 14{,}74°
  \label{eq:delta_max_calc}
\end{equation}

Contrairement à $\varepsilon_{\text{max}}$, atteint à la borne $\alpha=180°$
où la goupille active change, ce maximum est atteint à l'intérieur du
demi-tour ($\alpha\approx104{,}7°$)~: c'est donc un maximum lisse de l'écart
de position, pas un artefact du changement de goupille. Il se répète à
l'identique (même signe) à $\alpha\approx284{,}7°$~: la tige est ainsi
toujours en retard sur le moteur, jamais en avance.

\subsubsection{Conversion en imprécision temporelle}

L'accouplement se situant sur la tige et non directement sur le
barillet, la conversion de $\Delta\theta_{\text{max}}$ en temps doit passer par le
...
\begin{equation}
  \Delta t_{\text{max}} = \Delta\theta_{\text{max}}[°] \times \frac{10\cdot RM}{N_{\text{tige}}}
  \label{eq:dt_max}
\end{equation}

Le cas le plus défavorable est le Mvt01 ($RM = 80$~h, $N_{\text{total}} =
10{,}3$~tr, $r = 3{,}33$). Par l'équation~\eqref{eq:n_tige} :
...
puis, par l'équation~\eqref{eq:dt_max} avec $\Delta\theta_{\text{max}} = 14{,}74°$
(équation~\eqref{eq:delta_max_calc}) :
...
$\leq 15\ \text{min}$. En inversant l'équation~\eqref{eq:dt_max} (avec
l'équation~\eqref{eq:delta_max}), on obtient le désalignement radial $e$ qui amènerait
...
(équations~\eqref{eq:n_tige}, \eqref{eq:dt_max} et \eqref{eq:e_15min}), les
résultats pour l'ensemble des mouvements, avec $\Delta\theta_{\text{max}} = 14{,}74°$
(équation~\eqref{eq:delta_max_calc}, indépendant du mouvement puisqu'il ne
dépend que de $e$ et $R$).
...
charges, mais avec une marge restreinte (62 à 78~\%). Le mouvement Mvt01 est le
