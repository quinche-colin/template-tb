...
ARMAGE, pour « Acquisition de Réserve de Marche Accélérée avec Gestion de
l'Entraînement », est le nom donné au logiciel développé dans le cadre de ce
travail. Au-delà de l'acronyme, ce nom est aussi un clin d'œil au cliquet
d'armage (section~\ref{sec:mesure_acceleree}), la pièce du mouvement qu'il
faut retirer pour pouvoir dévider le barillet par la tige de remontoir.

\section{Organisation du dossier ARMAGE}

Le dossier \texttt{ARMAGE}, représenté sur la figure~\ref{fig:armage-arborescence},
contient l'ensemble du code source de l'application, ainsi que les
...
Le fichier \texttt{main.py} initialise l'application avant de l'ouvrir. Le
reste du programme est ensuite séparé en plusieurs dossiers, chacun dédié à
une fonction précise~: un pour piloter la caméra (\texttt{camera/}), un
pour piloter le moteur (\texttt{motor/}), un pour le traitement d'image
(\texttt{processing/}), un pour l'interface du logiciel (\texttt{gui/}), et
un pour les fonctions transverses comme la configuration ou la génération
des rapports PDF (\texttt{utils/}). Cette séparation facilite la
compréhension et le dépannage du code, en évitant d'avoir à parcourir
plusieurs milliers de lignes avant de trouver la fonction à dépanner.

Le dossier contient enfin un certain nombre de fichiers annexes~: des PDF
que l'application peut ouvrir (\texttt{Mode\_emploi\_ARMAGE.pdf},
\texttt{README.md}), des fichiers pour stocker des paramètres ou des
journaux d'exécution (\texttt{settings.json}, \texttt{app.log}), et les
fichiers servant à installer ou empaqueter l'application
(\texttt{requirements.txt}, \texttt{ARMAGE.spec},
\texttt{build\_windows.bat}, \texttt{build\_linux.sh}, \texttt{GxIAPI.dll}).

\section{Installation et utilisation}

Deux étapes sont nécessaires avant de pouvoir utiliser l'application sur un
nouveau poste.

La première consiste à installer les pilotes du port USB de la caméra, sans
lesquels l'ordinateur ne la reconnaît pas. Ces pilotes sont fournis avec le
Galaxy SDK du fabricant Daheng Imaging, téléchargeable sur son site
\cite{daheng_galaxy_sdk}~: lors de l'installation, il suffit de sélectionner
les pilotes correspondant à une caméra USB3.

La seconde consiste simplement à récupérer l'exécutable \texttt{ARMAGE}
généré dans le dossier \texttt{dist/} (figure~\ref{fig:armage-arborescence})
à l'issue de l'empaquetage de l'application, puis à le placer à
l'emplacement souhaité sur l'ordinateur~: aucune installation supplémentaire
n'est requise, l'application peut être lancée directement depuis cet
exécutable.
...
Avant de lancer une mesure, l'opérateur renseigne dans l'interface cinq
informations obligatoires : la référence du test, le numéro de série de la
montre et le nom de l'opérateur, saisis librement dans des champs textuels,
ainsi que le type de mouvement horloger et le mode de test (réserve de
marche ou vieillissement), sélectionnés depuis des menus déroulants qui
proposent toujours une valeur par défaut et ne peuvent donc jamais rester
vides. Le bouton de démarrage reste désactivé tant que les trois champs
textuels ne sont pas remplis, empêchant tout lancement sans identification
complète du test. Ces informations sont ensuite intégrées au rapport PDF
généré en fin de mesure.

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{assets/figures/Resultats/interface_module.png}
    \caption{Champs de saisie des informations de test dans l'interface}
    \label{fig:gui_saisie_test}
\end{figure}
...
Lorsqu'un mouvement est sélectionné, ses paramètres sont chargés
automatiquement au lancement du test (sans être affichés à l'écran),
garantissant que les calculs de tours et de couple restent cohérents même
si la sélection est modifiée après une première tentative.

Cette liste peut être modifiée par l'opérateur directement depuis
l'interface~: le menu glissant sur la droite de l'application
(figure~\ref{fig:gui_menu}) donne accès à la fenêtre « Gestion des
mouvements » (figure~\ref{fig:gui_mouvements}), qui permet d'ajouter ou de
supprimer un mouvement de la liste. Les mouvements ajoutés sont enregistrés
dans un fichier de configuration JSON et rechargés automatiquement à chaque
démarrage de l'application, au même titre que les mouvements intégrés.

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{assets/figures/Implementation/menu_navigation.png}
    \caption{Menu latéral de navigation de l'application}
    \label{fig:gui_menu}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{assets/figures/Implementation/gestion_mouvements.png}
    \caption{Fenêtre de gestion des mouvements}
    \label{fig:gui_mouvements}
\end{figure}
...
La figure~\ref{fig:image_camera} montre une image telle que capturée par la
caméra avec ces réglages, sur laquelle se base l'algorithme de détection
(section~\ref{sec:detection}). L'horodatage est incrusté directement par la
caméra en haut de l'image, et la teinte bleutée provient du filtre
polarisant croisé utilisé pour supprimer les reflets spéculaires
(figure~\ref{fig:filtres_polarisants}).

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{assets/figures/Implementation/image_mouvement_par_camera.jpg}
    \caption{Image du mouvement telle que capturée par la caméra}
...
matériel connecté. Une fenêtre dédiée (figure~\ref{fig:gui_connexion})
permet par ailleurs de forcer une reconnexion du moteur ou de la caméra,
par exemple après un débranchement.

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{assets/figures/Implementation/connexion_appareils.png}
    \caption{Fenêtre de connexion des appareils}
    \label{fig:gui_connexion}
\end{figure}
...
avantages : il ne nécessite aucun câblage supplémentaire ni
...
est ajoutée directement sur l'image sous la forme d'une bande
semi-transparente en haut du cadre, affichant la date et l'heure au
...
automatique de l'heure de prise de vue à partir uniquement du nom de fichier.
...
Cela signifie que les aiguilles occupent la même position à $t_n$ et à $t_{n-1}$, ce qui signifie que la montre s'est arrêtée dans l'intervalle $[t_{n-2},\, t_{n-1}]$. En effet, la comparaison précédente, entre $I_{n-2}$ et $I_{n-1}$, avait nécessairement donné un MSE supérieur au seuil (sans quoi l'arrêt aurait déjà été détecté à cette étape précédente), ce qui prouve que les aiguilles se sont bien déplacées entre $t_{n-2}$ et $t_{n-1}$, mais sans indiquer à quel instant précis de cet intervalle ce déplacement a eu lieu.
