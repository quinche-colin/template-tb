...
préparatoire en trois étapes successives : l'accouplement, le remontage et
le dévidage. Ces étapes sont pilotées par un moteur pas à pas TMCM-1021
(Trinamic) connecté au système via une liaison RS485.

\subsection{Étape 1 : accouplement}

L'accouplement est une phase préliminaire dont le but est d'engager
mécaniquement le moteur avec le mécanisme de remontoir de la montre avant
d'appliquer un couple significatif. Le moteur effectue une rotation horaire
de 0{,}5 tour à vitesse réduite (10~tr/min) et à faible couple
($\approx 6$~mN$\cdot$m), insuffisant pour entraîner le barillet mais
suffisant pour engager le mécanisme d'accouplement.

Cette précaution évite un choc mécanique lors du démarrage du remontage, qui
pourrait endommager l'accouplement, mais surtout cela garanti que le nombre de tours effectué par le moteur soit le même que celui effectué par la tige du mouvement.

\subsection{Étape 2 : remontage du barillet}
...
\subsection{Étape 3 : dévidage partiel du barillet}
...
fourche–T reste en place et la tige du mouvement demeure solidaire de l'arbre
...
\begin{itemize}
    \item \textbf{Seuil trop élevé :} risque de faux positifs. Un déplacement
    ...
    \item \textbf{Seuil trop faible :} risque de faux négatifs. Le bruit
